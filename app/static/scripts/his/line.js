DrawLine={
	...
	/**
	* xï¼šåˆ—å¼€å§‹å¤„,yï¼šè¡Œå¼€å§‹å¤„,wï¼šç»„ä¸Žç»„ä¹‹é—´çš„è·ç¦»,mbï¼šä¸Žåº•éƒ¨é—´çš„è·ç¦»
	* æŠŠæ¯ä¸€å—å°æˆç»„åŠ ä¸Šå±žæ€§
	*/
...
JoinLine.prototype={
	...
	FFLine:function(x1,y1,x2,y2){
		if(Math.abs(y1-y2)<(JoinLine.indent*2)&&x1==x2)return;//è‡ªåŠ¨ç¡®å®šåŒåˆ—çš„æ˜¯å¦è¿žçº¿		
		var np=this.nPos(x1,y1,x2,y2,JoinLine.indent);//ä¸¤ç«¯ç¼©å‡å‡½æ•°ï¼ˆé˜²æ­¢è¦†ç›–çƒï¼‰
...
LG=function(table,_x,_y,width,margin_bottom,fn_check){
	...
	this.g=[];
	for(var i=row_start;i<row_end;i++){/* æ£€æµ‹æ¯ä¸€ç»„å›¾å½¢ */
		var tr=trs[i].cells;
		for(var j=col_start;j<col_end;j++){
			var td=tr[j];
			/* æ£€æµ‹å™¨è¿”å›žç»å¯¹çœŸæ—¶ï¼Œå•å…ƒæ ¼æ‰è¢«æ·»åŠ åˆ°ç»„ */
			if(td){
...
LG.color='#E4A8A8';/*æŠ˜çº¿é»˜è®¤è‰²*/
LG.size=2;/*æŠ˜çº¿å®½åº¦*/
LG.autoDraw=true;/* é»˜è®¤è‡ªåŠ¨ç»˜çº¿ */
LG.isShow=true;
LG.prototype={
	draw:function(){
...
/* é‡å†™fw.onReady å»¶è¿Ÿæ‰§è¡Œåˆ°window.onload */
fw={}
